...
class ReportController extends Controller
{
    ...
    public function sale(Request $request)
    {
        ...
        $dateToDisplay = formatDateInIndonesian($currentDate, 'd F Y');

        // Set start and end date with default values if not provided
        $startDate = $request->start_date
            ? Carbon::parse($request->start_date, $timezone)->setTimezone($timezone)
            : Carbon::parse(now($timezone)->setTime(12, 0)->format('Y-m-d H:i:s'));
        $endDate = $request->end_date
            ? Carbon::parse($request->end_date, $timezone)->setTimezone($timezone)
            : Carbon::parse(now($timezone)->addDay()->setTime(12, 0)->format('Y-m-d H:i:s'));

        if ($request->start_date)  $dateToDisplay = formatDateInIndonesian($request->start_date, 'd F Y') . ' - ' . formatDateInIndonesian($request->end_date, 'd F Y');
        ...
        $buybackQty = $dataBuyback->sum('qty');
        $buybackGramasi = $dataBuyback->sum('gramasi');
        $buybackTotal = $dataBuyback->sum('total');

        // Calculate return data
        $dataReturn = $this->getReturnData($startDate, $endDate);
        $returnQty = $dataReturn->sum('qty');
        $returnGramasi = $dataReturn->sum('gramasi');

        // Return view with data if needed
        return view('report.sale_report', compact('dataSale', 'dataBuyback', 'dataReturn', 'saleQty', 'saleGramasi', 'saleTotal', 'buybackQty', 'buybackGramasi', 'buybackTotal', 'returnQty', 'returnGramasi', 'startDate', 'endDate', 'dateToDisplay'));
    }

    /**
     * Get Sale Data
     */
    private function getSaleData($startDate, $endDate)
    {
        return Product_Sale::select([
            'categories.name as category_name',
            DB::raw('SUM(gramasis.gramasi) as gramasi'),
            DB::raw('COUNT(products.category_id) as qty'),
            DB::raw('SUM(product_sales.total) as total')
        ])
            ->join('products', 'product_sales.product_id', '=', 'products.id')
            ->join('gramasis', 'products.gramasi_id', '=', 'gramasis.id')
     ...
     * Get Buyback Data
     */
    private function getBuybackData($startDate, $endDate)
    {
        return ProductBuyback::select([
            DB::raw('SUM(gramasis.gramasi) as gramasi'),
            DB::raw('COUNT(products.category_id) as qty'),
            DB::raw('SUM(product_buyback.final_price) as total')
        ])
            ->join('products', 'product_buyback.product_id', '=', 'products.id')
            ->join('gramasis', 'products.gramasi_id', '=', 'gramasis.id')
            ->whereBetween('product_buyback.created_at', [$startDate, $endDate])
            ->get();
    }
     ...
     * Get Return Data
     */
    private function getReturnData($startDate, $endDate)
    {
        return WarehouseTransfer::select([
            DB::raw('SUM(gramasis.gramasi) as gramasi'),
            DB::raw('COUNT(products.category_id) as qty'),
        ])
            ->join('products', 'warehouse_transfers.product_id', '=', 'products.id')
            ->join('gramasis', 'products.gramasi_id', '=', 'gramasis.id')
            ->whereBetween('warehouse_transfers.created_at', [$startDate, $endDate])
            ->get();
    }
