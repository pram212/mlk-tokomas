...
class SaleController extends Controller
{
            ...
            foreach ($sales as $key => $sale) {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="' . route('sales.invoice', $sale->id) . '" class="btn btn-link"><i class="fa fa-copy"></i> ' . trans('file.Generate Invoice') . '</a></li>
                                <li>
    ...
    public function store(Request $request)
    {
        ...
        $data['total_price'] = (str_replace('.', '', $data['total_price']) == '' ? 0 : str_replace('.', '', $data['total_price']));
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_sale['discount'] = (str_replace('.', '', $discount[$i]) == '' ? 0 : str_replace('.', '', $discount[$i]));
        ...
        if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            $lims_payment_data->amount = (str_replace('.', '', $data['paid_amount']) == '' ? 0 : str_replace('.', '', $data['paid_amount']));
            $lims_payment_data->change = (str_replace('.', '', $data['paying_amount']) == '' ? 0 : str_replace('.', '', $data['paying_amount'])) - (str_replace('.', '', $data['paid_amount']) == '' ? 0 : str_replace('.', '', $data['paid_amount']));
    ...
    public function getProduct($id)
    {
        ...
        $lims_product_data = Product::limit(10)->get();
    ...
    public function saleByCsv()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('sales.import', compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function addPayment(Request $request)
    {
        ...
        $data['paying_amount'] = (str_replace('.', '', $data['paying_amount']) == '' ? 0 : str_replace('.', '', $data['paying_amount']));
    ...
    public function updatePayment(Request $request)
    {
        ...
        $lims_customer_data = Customer::find($lims_sale_data->customer_id);
        //updating sale table
        $data['edit_amount'] = (str_replace('.', '', $data['edit_amount']) == '' ? 0 : str_replace('.', '', $data['edit_amount']));
        $data['edit_paying_amount'] = (str_replace('.', '', $data['edit_paying_amount']) == '' ? 0 : str_replace('.', '', $data['edit_paying_amount']));
