						    ...
						    <tr>
						        ...
						        <td>
						        	@if($discount > 0)
							      	...
							      	<span>{{number_format($discount,0, ',' , '.') }}</span><br><br>
							      	@endif
							      	@if($order_discount[$key] > 0)
							      	<strong>{{trans("file.Order Discount")}}</strong><br>
							      	<span>{{number_format($order_discount[$key],0, ',' , '.')}}</span><br><br>
							      	@endif
							      	@if($total_tax[$key] > 0)
							      	<strong>{{trans("file.Product Tax")}}</strong><br>
							      	<span>{{number_format($total_tax[$key],0, ',' , '.')}}</span><br><br>
							      	@endif
							      	@if($order_tax[$key] > 0)
							      	<strong>{{trans("file.Order Tax")}}</strong><br>
							      	<span>{{number_format($order_tax[$key],0, ',' , '.')}}</span><br><br>
							      	@endif
							      	@if($shipping_cost[$key] > 0)
							      	<strong>{{trans("file.Shipping Cost")}}</strong><br>
							      	<span>{{number_format($shipping_cost[$key],0, ',' , '.')}}</span><br><br>
							      	@endif
							      	@if($grand_total[$key] > 0)
							      	<strong>{{trans("file.grand total")}}</strong><br>
							      	<span>{{number_format($grand_total[$key],0, ',' , '.') }}</span><br>
							      	@endif
						        </td>
						        @endforeach
						    </tr>
