                    ...
                    <tbody>
                        @foreach($lims_payroll_data as $key=>$payroll)
                            <tr>
                                ...
                                <td>{{number_format($payroll->amount,0, ',' , '.') }}</td>
...
<script type="text/javascript">
    ...
    function datatable_sum_sale(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            const data4 = dt_selector.cells( rows, 7, { page: 'current' } ).data();
            var dataSum4 =0 ;
            for(var i=0; i < data4.length; i++) {
                dataSum4 += Number(data4[i].replaceAll('.', '') == '' ? 0 : data4[i].replaceAll('.', ''));
            }

            const data6 = dt_selector.cells( rows, 8, { page: 'current' } ).data();
            var dataSum6 =0 ;
            ...
            $( dt_selector.column( 7 ).footer() ).html(formatRupiah(dataSum4));
            $( dt_selector.column( 8 ).footer() ).html(formatRupiah(dataSum6));
        }
        else {
            ...
            const data4 = dt_selector.column( 7, {page:'current'} ).data();
            var dataSum4 =0 ;
            for(var i=0; i < data4.length; i++) {
                dataSum4 += Number(data4[i].replaceAll('.', '') == '' ? 0 : data4[i].replaceAll('.', ''));
            }

            const data6 = dt_selector.column( 8, {page:'current'} ).data();
            var dataSum6 =0 ;
            for(var i=0; i < data6.length; i++) {
                dataSum6 += Number(data6[i].replaceAll('.', '') == '' ? 0 : data6[i].replaceAll('.', ''));
            }
    ...
    function datatable_sum_purchase(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            $( dt_selector.column( 6 ).footer() ).html(formatRupiah(dataSum2));
            $( dt_selector.column( 7 ).footer() ).html(formatRupiah(dataSum4));
            $( dt_selector.column( 8 ).footer() ).html(formatRupiah(dataSum2));
        }
        else {
    ...
    function datatable_sum_payment(dt_selector, is_calling_first) {
        if (dt_selector.rows( '.selected' ).any() && is_calling_first) {
            ...
            const data2 = dt_selector.cells( rows, 3, { page: 'current' } ).data();
            var dataSum2 =0 ;
            for(var i=0; i < data2.length; i++) {
                dataSum2 += Number(data2[i].replaceAll('.', '') == '' ? 0 : data2[i].replaceAll('.', ''));
            }

            $( dt_selector.column( 3 ).footer() ).html(formatRupiah(dataSum2));
        }
        else {

            const data2 = dt_selector.column( 3, {page:'current'} ).data();
            var dataSum2 =0 ;
            for(var i=0; i < data2.length; i++) {
                dataSum2 += Number(data2[i].replaceAll('.', '') == '' ? 0 : data2[i].replaceAll('.', ''));
            }
